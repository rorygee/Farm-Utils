...
/** Quest patch observer for the Song of the Elves elder cadantine patch (varbit 9016). */
@Slf4j
@Singleton
public class Varbit9016ElderCadantineVarbitObserver
{
    static final int VARBIT_ID = 9016;

    private static final int REGION_ID = 9265;
    private static final int PLANE = 0;
    private static final WorldPoint ANCHOR = new WorldPoint(2321, 3151, 0);
    ...
    @Inject
    public Varbit9016ElderCadantineVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    public void onGameTick()
    {
        ...
        handleVarbitValue(PatchId.QUEST_ELDER_CADANTINE, raw);
    }

    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        final Instant now = Instant.now();
        final DecodedPatchState cur = QuestElderCadantineSlotDecoder.decode(raw);
        ...
        // Planting (quest action): empty -> stage 1.
