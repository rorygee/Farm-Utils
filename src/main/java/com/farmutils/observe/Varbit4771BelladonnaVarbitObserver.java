...
/** Belladonna patch observer for FARMING_TRANSMIT_A (varbit 4771) at Draynor Manor. */
@Slf4j
@Singleton
public class Varbit4771BelladonnaVarbitObserver
{
    ...
    @Inject
    public Varbit4771BelladonnaVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        ...
        // Planting: empty -> stage 1 (use fully-raked sentinel raw=3).
    ...
    private PatchId resolveAttributedPatch()
    {
        ...
        final int regionId = p.getWorldLocation().getRegionID();
    ...
    static
    {
        // Draynor Manor belladonna patch (RuneLite Time Tracking: region 12340)
        REGION_ID_TO_PATCH.put(12340, PatchId.SPECIAL_BELLADONNA_DRAYNOR);
    }
}
