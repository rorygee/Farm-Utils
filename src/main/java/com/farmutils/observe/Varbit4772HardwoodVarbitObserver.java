...
/** Hardwood tree patch observer for FARMING_TRANSMIT_B (varbit 4772) on Fossil Island. */
@Slf4j
@Singleton
public class Varbit4772HardwoodVarbitObserver
{
    static final int VARBIT_ID = 4772;
    private static final Duration STAGE_CYCLE = Duration.ofMinutes(640);

    private static final int FOSSIL_ISLAND_REGION_ID = 14651;
    private static final PatchId PATCH_ID = PatchId.SPECIAL_TREE_HARDWOOD_FOSSIL_ISLAND_PLOT_2;
    ...
    @Inject
    public Varbit4772HardwoodVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    public void onGameTick()
    {
        ...
        if (p.getWorldLocation().getRegionID() != FOSSIL_ISLAND_REGION_ID)
        {
            reset();
            return;
        }
    ...
    {
        return regionId == FOSSIL_ISLAND_REGION_ID ? PATCH_ID : null;
    }
}
