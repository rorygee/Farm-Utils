...
/** Spirit tree observer for FARMING_TRANSMIT_A (varbit 4771). */
@Slf4j
@Singleton
public class Varbit4771SpiritTreeVarbitObserver
{
    static final int VARBIT_ID = 4771;
    private static final Duration STAGE_CYCLE = Duration.ofMinutes(320);

    // Farming Guild proximity gate (spirit tree is sent on 4771 inside the guild).
    ...
    @Inject
    public Varbit4771SpiritTreeVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        ...
        final DecodedPatchState cur = StandardSpiritTreeSlotDecoder.decode(raw);
    ...
    private PatchId resolveAttributedPatch()
    {
        ...
        // Farming Guild spirit tree is reliably attributable by proximity.
        if (here.getPlane() == FARMING_GUILD_ANCHOR.getPlane() && here.distanceTo2D(FARMING_GUILD_ANCHOR) <= MAX_ATTRIBUTION_DISTANCE_TILES)
        {
            return PatchId.SPIRIT_TREE_FARMING_GUILD;
        }

        // Port Sarim spirit tree shares a region boundary with Falador allotments; use the same y<3272 gate.
        int regionId = here.getRegionID();
        if ((regionId == 12082 || regionId == 12083) && here.getY() < 3272)
        {
            return PatchId.SPIRIT_TREE_PORT_SARIM;
        }
    ...
    {
        if (regionId == 12082 || regionId == 12083)
        {
            return PatchId.SPIRIT_TREE_PORT_SARIM;
        }
        // Farming Guild is proximity-based.
        return null;
    }
}
