...
/** Bush patch observer for FARMING_TRANSMIT_A (varbit 4771). */
@Slf4j
@Singleton
public class Varbit4771BushVarbitObserver
{
    static final int VARBIT_ID = 4771;
    private static final Duration STAGE_CYCLE = Duration.ofMinutes(20);
    ...
    @Inject
    public Varbit4771BushVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        ...
        final DecodedPatchState cur = StandardBushSlotDecoder.decode(raw);
    ...
    {
        // Champions' Guild
        REGION_ID_TO_PATCH.put(12596, PatchId.BUSH_CHAMPIONS_GUILD);

        // Rimmington (+ adjacent)
        REGION_ID_TO_PATCH.put(11570, PatchId.BUSH_RIMMINGTON);
        REGION_ID_TO_PATCH.put(11826, PatchId.BUSH_RIMMINGTON);

        // Ardougne (+ adjacent)
        REGION_ID_TO_PATCH.put(10290, PatchId.BUSH_ARDOUGNE);
        REGION_ID_TO_PATCH.put(10546, PatchId.BUSH_ARDOUGNE);

        // Etceteria
        REGION_ID_TO_PATCH.put(10300, PatchId.BUSH_ETCETERIA);
    }
}
