...
/** Hardwood tree patch observer for FARMING_TRANSMIT_A (varbit 4771). */
@Slf4j
@Singleton
public class Varbit4771HardwoodVarbitObserver
{
    ...
    @Inject
    public Varbit4771HardwoodVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        ...
        final DecodedPatchState cur = StandardHardwoodSlotDecoder.decode(raw);
    ...
    static
    {
        // Fossil Island hardwood (plot 1)
        REGION_ID_TO_PATCH.put(14651, PatchId.SPECIAL_TREE_HARDWOOD_FOSSIL_ISLAND_PLOT_1);

        // Anglers' Retreat hardwood
        REGION_ID_TO_PATCH.put(9770, PatchId.SPECIAL_TREE_HARDWOOD_ANGLERS_RETREAT);

        // Varlamore hardwood (Time Tracking: Avium Savannah)
        REGION_ID_TO_PATCH.put(6702, PatchId.SPECIAL_TREE_HARDWOOD_LOCUS_OASIS);
        REGION_ID_TO_PATCH.put(6446, PatchId.SPECIAL_TREE_HARDWOOD_LOCUS_OASIS);
    }
}
