...
/** Quest patch observer for the Grim Tales magic beanstalk (varbit 3714). */
@Slf4j
@Singleton
public class Varbit3714MagicBeansVarbitObserver
{
    static final int VARBIT_ID = 3714;

    private static final int REGION_ID = 11573;
    private static final int PLANE = 0;
    private static final WorldPoint ANCHOR = new WorldPoint(2921, 3424, 0);
    ...
    @Inject
    public Varbit3714MagicBeansVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    public void onGameTick()
    {
        ...
        handleVarbitValue(PatchId.QUEST_MAGIC_BEANS, raw);
    }

    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        final Instant now = Instant.now();
        final DecodedPatchState cur = QuestMagicBeansSlotDecoder.decode(raw);
        ...
        // Harvest/quest completion: crop -> empty (stump).
    ...
    private static boolean isNaturalGrowthTickTransition(final DecodedPatchState prev, final DecodedPatchState cur)
    {
        ...
        return cur.getStage() - prev.getStage() == 1;
    }
    ...
    private void emitObservedStateIfUseful(final PatchId patch, final Instant now, final DecodedPatchState cur, final boolean stageTickTransition)
    {
        ...
        if (!cur.isEmpty() && cropName != null)
        {
            // No stable item id; keep icon generic but still preserve the name.
            if (cropItemId != null)
            {
