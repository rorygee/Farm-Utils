...
/**
 * Allotment patch observer for FARMING_TRANSMIT_A (varbit 4771).
 *
 * <p>This slot is shared across many allotment locations (plot 1). We attribute by region id +
 * proximity to an anchor point, mirroring the flower/herb observer pattern.</p>
 */
@Slf4j
@Singleton
public class Varbit4771AllotmentVarbitObserver
{
    static final int VARBIT_ID = 4771;

    // Reuse the farming-area anchors already in use for herb/flower observers.
    ...
    // Harmony Island is a single-allotment location. Anchor is best-effort; refine via Varbit Explorer if needed.
    static final WorldPoint HARMONY_ISLAND_ANCHOR = new WorldPoint(3808, 2848, 0);
    ...
    @Inject
    public Varbit4771AllotmentVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    {
        // Ardougne (Hemenster)
        registerRegion(10548, PatchId.ALLOTMENT_ARDOUGNE_PLOT_1, ARDOUGNE_ANCHOR);

        // Catherby (+ adjacent regions)
        registerRegion(11062, PatchId.ALLOTMENT_CATHERBY_PLOT_1, CATHERBY_ANCHOR);
        registerRegion(11061, PatchId.ALLOTMENT_CATHERBY_PLOT_1, CATHERBY_ANCHOR);
        registerRegion(11318, PatchId.ALLOTMENT_CATHERBY_PLOT_1, CATHERBY_ANCHOR);
        registerRegion(11317, PatchId.ALLOTMENT_CATHERBY_PLOT_1, CATHERBY_ANCHOR);

        // Civitas illa Fortis (+ adjacent regions)
        registerRegion(6192, PatchId.ALLOTMENT_CIVITAS_ILLA_FORTIS_PLOT_1, CIVITAS_ILLA_FORTIS_ANCHOR);
        registerRegion(6447, PatchId.ALLOTMENT_CIVITAS_ILLA_FORTIS_PLOT_1, CIVITAS_ILLA_FORTIS_ANCHOR);
        registerRegion(6448, PatchId.ALLOTMENT_CIVITAS_ILLA_FORTIS_PLOT_1, CIVITAS_ILLA_FORTIS_ANCHOR);
        registerRegion(6449, PatchId.ALLOTMENT_CIVITAS_ILLA_FORTIS_PLOT_1, CIVITAS_ILLA_FORTIS_ANCHOR);
        registerRegion(6191, PatchId.ALLOTMENT_CIVITAS_ILLA_FORTIS_PLOT_1, CIVITAS_ILLA_FORTIS_ANCHOR);
        registerRegion(6193, PatchId.ALLOTMENT_CIVITAS_ILLA_FORTIS_PLOT_1, CIVITAS_ILLA_FORTIS_ANCHOR);

        // Falador
        registerRegion(12083, PatchId.ALLOTMENT_FALADOR_PLOT_1, FALADOR_ANCHOR);

        // Hosidius (Kourend)
        registerRegion(6967, PatchId.ALLOTMENT_HOSIDIUS_PLOT_1, HOSIDIUS_ANCHOR);
        registerRegion(6711, PatchId.ALLOTMENT_HOSIDIUS_PLOT_1, HOSIDIUS_ANCHOR);

        // Port Phasmatys
        registerRegion(14391, PatchId.ALLOTMENT_PORT_PHASMATYS_PLOT_1, PHASMATYS_ANCHOR);
        registerRegion(14390, PatchId.ALLOTMENT_PORT_PHASMATYS_PLOT_1, PHASMATYS_ANCHOR);

        // Prifddinas (+ underground adjacency)
        registerRegion(13151, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12895, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12894, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(13150, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12994, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12993, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12737, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12738, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12126, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(12127, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);
        registerRegion(13250, PatchId.ALLOTMENT_PRIFDDINAS_PLOT_1, PRIFDDINAS_ANCHOR);

        // Harmony Island
        registerRegion(15148, PatchId.ALLOTMENT_HARMONY_ISLAND, HARMONY_ISLAND_ANCHOR);
    }
}
