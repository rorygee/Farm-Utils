...
/** Calquat patch observer for FARMING_TRANSMIT_A (varbit 4771) at Tai Bwo Wannai + Kastori. */
@Slf4j
@Singleton
public class Varbit4771CalquatVarbitObserver
{
    ...
    @Inject
    public Varbit4771CalquatVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    {
        // Tai Bwo Wannai
        REGION_ID_TO_PATCH.put(11056, PatchId.SPECIAL_TREE_CALQUAT_TAI_BWO_WANNAI);

        // Kastori (+ adjacent)
        REGION_ID_TO_PATCH.put(5423, PatchId.SPECIAL_TREE_CALQUAT_KASTORI);
        REGION_ID_TO_PATCH.put(5167, PatchId.SPECIAL_TREE_CALQUAT_KASTORI);
        REGION_ID_TO_PATCH.put(5424, PatchId.SPECIAL_TREE_CALQUAT_KASTORI);
    }
}
