...
/** Quest patch observer for Kelda hops (varbit 823). */
@Slf4j
@Singleton
public class Varbit823KeldaHopsVarbitObserver
{
    static final int VARBIT_ID = 823;

    private static final int REGION_ID = 11423;
    private static final int PLANE = 0;
    private static final WorldPoint ANCHOR = new WorldPoint(2853, 10206, 0);
    ...
    @Inject
    public Varbit823KeldaHopsVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    public void onGameTick()
    {
        ...
        handleVarbitValue(PatchId.QUEST_KELDA_HOPS, raw);
    }

    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        final Instant now = Instant.now();
        final DecodedPatchState cur = QuestKeldaHopsSlotDecoder.decode(raw);
        ...
        // Harvest/clear: crop -> weeds/empty.
