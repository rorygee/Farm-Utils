...
/** Cactus patch observer for FARMING_TRANSMIT_F (varbit 7904) at Farming Guild. */
@Slf4j
@Singleton
public class Varbit7904CactusVarbitObserver
{
    static final int VARBIT_ID = 7904;

    private static final Duration CACTUS_CYCLE = Duration.ofMinutes(80);
    private static final Duration POTATO_CACTUS_CYCLE = Duration.ofMinutes(10);
    ...
    private static final PatchId PATCH_ID = PatchId.CACTUS_FARMING_GUILD;
    ...
    @Inject
    public Varbit7904CactusVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final int raw)
    {
        ...
        final DecodedPatchState cur = StandardCactusSlotDecoder.decode(raw);
    ...
    private static Duration durationForCrop(final DecodedPatchState cur)
    {
        Integer itemId = cur != null ? cur.getCropItemIdOrNull() : null;
        if (itemId != null && itemId == ItemID.CACTUS_POTATO)
        {
            return POTATO_CACTUS_CYCLE;
        }
        return CACTUS_CYCLE;
    }
    ...
    private void emitObservedStateIfUseful(final Instant now, final DecodedPatchState cur, final boolean stageTickTransition)
    {
        if (!cur.isEmpty() && cur.getHealth() == PatchHealth.HEALTHY && cur.getStage() >= 1)
        {
            ...
            Duration cycle = durationForCrop(cur);

            if (stageTickTransition)
            {
                inferenceEngine.onObservation(Observation.growthStageTransition(PATCH_ID, cur.getStage(), maxStageOrNull, cycle, now, ObservationSource.VARBIT));
            }
            else
            {
                inferenceEngine.onObservation(Observation.growthStageObserved(PATCH_ID, cur.getStage(), maxStageOrNull, cycle, now, ObservationSource.VARBIT));
            }
    ...
    {
        // Not region-gated here; scope is proximity-based.
        return PATCH_ID;
    }
}
