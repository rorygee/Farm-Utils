...
/** Belladonna patch observer for FARMING_TRANSMIT_B (varbit 4772) at Auburnvale. */
@Slf4j
@Singleton
public class Varbit4772BelladonnaVarbitObserver
{
    ...
    @Inject
    public Varbit4772BelladonnaVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final PatchId patch, final int raw)
    {
        ...
        final DecodedPatchState cur = StandardBelladonnaSlotDecoder.decode(raw);
    ...
    static
    {
        // Auburnvale belladonna patch (RuneLite Time Tracking: 5427 + adjacent)
        REGION_ID_TO_PATCH.put(5427, PatchId.SPECIAL_BELLADONNA_AUBURNVALE);
        REGION_ID_TO_PATCH.put(5428, PatchId.SPECIAL_BELLADONNA_AUBURNVALE);
        REGION_ID_TO_PATCH.put(5684, PatchId.SPECIAL_BELLADONNA_AUBURNVALE);
    }
}
