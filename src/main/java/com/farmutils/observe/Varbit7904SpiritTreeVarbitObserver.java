...
/** Spirit tree observer for FARMING_TRANSMIT_F (varbit 7904) in Hosidius (Kourend). */
@Slf4j
@Singleton
public class Varbit7904SpiritTreeVarbitObserver
{
    static final int VARBIT_ID = 7904;
    private static final Duration STAGE_CYCLE = Duration.ofMinutes(320);

    private static final PatchId PATCH_ID = PatchId.SPIRIT_TREE_HOSIDIUS;
    ...
    @Inject
    public Varbit7904SpiritTreeVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    public void onGameTick()
    {
        ...
        if (!HOSIDIUS_REGION_IDS.contains(p.getWorldLocation().getRegionID()))
        {
            reset();
            return;
        }
    ...
    {
        return HOSIDIUS_REGION_IDS.contains(regionId) ? PATCH_ID : null;
    }

    private static final Set<Integer> HOSIDIUS_REGION_IDS = new HashSet<>();

    static
    {
        // Time Tracking: Kourend/Hosidius farming region ids.
        HOSIDIUS_REGION_IDS.add(6967);
        HOSIDIUS_REGION_IDS.add(6711);
    }
}
