...
/** Celastrus patch observer for FARMING_TRANSMIT_L (varbit 7910) at Farming Guild. */
@Slf4j
@Singleton
public class Varbit7910CelastrusVarbitObserver
{
    static final int VARBIT_ID = 7910;
    ...
    private static final PatchId PATCH_ID = PatchId.SPECIAL_TREE_CELASTRUS_FARMING_GUILD;
    ...
    @Inject
    public Varbit7910CelastrusVarbitObserver(final Client client, final InferenceEngine inferenceEngine)
    {
        this.client = client;
    ...
    private void handleVarbitValue(final int raw)
    {
        ...
        final DecodedPatchState cur = StandardCelastrusSlotDecoder.decode(raw);
    ...
    private void emitObservedStateIfUseful(final Instant now, final DecodedPatchState cur, final boolean stageTickTransition)
    {
        if (!cur.isEmpty() && cur.getHealth() == PatchHealth.HEALTHY && cur.getStage() >= 1)
        {
            ...
            // Capture harvest depletion stage for UI ("lives left").
            if (cur.getStage() == cur.getMaxGrowthStageOrZero())
            {
                int hs = StandardCelastrusSlotDecoder.getHarvestStageOrZero(cur.getRaw());
                if (hs > 0)
                {
                    inferenceEngine.onObservation(Observation.harvestStageObserved(PATCH_ID, hs, now, ObservationSource.VARBIT));
                }
            }
        }
    }
