...
public class BushPatchPipelineTest
{
    @Test
    public void bushDecodesAndFeedsInference()
    {
        assertEquals(PatchId.BUSH_CHAMPIONS_GUILD, Varbit4771BushVarbitObserver.patchForRegionId(12596));

        // Redberries stage 1
        DecodedPatchState stage1 = StandardBushSlotDecoder.decode(5);
        ...
        assertEquals(Integer.valueOf(ItemID.REDBERRIES), stage1.getCropItemIdOrNull());
        ...
        PatchId patch = PatchId.BUSH_CHAMPIONS_GUILD;
        engine.onObservation(Observation.planted(patch, t0, ObservationSource.VARBIT));
        engine.onObservation(Observation.growthStageObserved(patch, stage1.getStage(), stage1.getMaxGrowthStageOrZero(), Duration.ofMinutes(20), t0, ObservationSource.VARBIT));
        ...
        // Expected window for stage1 baseline: 100..120 minutes.
        clock.setNow(t0.plus(Duration.ofMinutes(100)));
        engine.tick();
        assertEquals(InferredStage.GROWING, engine.get(patch).getStage());
        assertEquals(t0.plus(Duration.ofMinutes(100)), engine.get(patch).getEarliestReadyAt());
        assertEquals(t0.plus(Duration.ofMinutes(120)), engine.get(patch).getLatestReadyAt());

        clock.setNow(t0.plus(Duration.ofMinutes(121)));
        engine.tick();
