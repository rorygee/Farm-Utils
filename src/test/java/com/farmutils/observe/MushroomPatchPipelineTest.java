...
public class MushroomPatchPipelineTest
{
    @Test
    public void mushroomDecodesAndFeedsInference()
    {
        assertEquals(PatchId.SPECIAL_MUSHROOM, Varbit4771MushroomVarbitObserver.patchForRegionId(13622));
        assertEquals(PatchId.SPECIAL_MUSHROOM, Varbit4771MushroomVarbitObserver.patchForRegionId(13878));

        // Mushroom stage 1
        DecodedPatchState stage1 = StandardMushroomSlotDecoder.decode(4);
        ...
        assertEquals(Integer.valueOf(ItemID.BITTERCAP_MUSHROOM), stage1.getCropItemIdOrNull());

        // Harvestable variant should map to max stage.
        DecodedPatchState harvestable = StandardMushroomSlotDecoder.decode(10);
        assertFalse(harvestable.isEmpty());
        assertEquals(7, harvestable.getStage());
        ...
        PatchId patch = PatchId.SPECIAL_MUSHROOM;
        ...
        // Stage 1/7 -> remaining 6 ticks @ 40 minutes.
        clock.setNow(t0.plus(Duration.ofMinutes(240)));
        engine.tick();
        assertEquals(InferredStage.GROWING, engine.get(patch).getStage());
        assertEquals(t0.plus(Duration.ofMinutes(240)), engine.get(patch).getEarliestReadyAt());
        assertEquals(t0.plus(Duration.ofMinutes(280)), engine.get(patch).getLatestReadyAt());

        clock.setNow(t0.plus(Duration.ofMinutes(281)));
        engine.tick();
