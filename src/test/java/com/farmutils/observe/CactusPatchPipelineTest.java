...
public class CactusPatchPipelineTest
{
    @Test
    public void potatoCactusDecodesAndFeedsInference()
    {
        assertEquals(PatchId.CACTUS_AL_KHARID, Varbit4771CactusVarbitObserver.patchForRegionId(13106));

        // Potato cactus stage 1
        DecodedPatchState stage1 = StandardCactusSlotDecoder.decode(32);
        ...
        assertEquals(Integer.valueOf(ItemID.CACTUS_POTATO), stage1.getCropItemIdOrNull());
        ...
        PatchId patch = PatchId.CACTUS_AL_KHARID;
        ...
        clock.setNow(t0.plus(Duration.ofMinutes(70)));
        engine.tick();
        assertEquals(InferredStage.GROWING, engine.get(patch).getStage());
        assertEquals(t0.plus(Duration.ofMinutes(70)), engine.get(patch).getEarliestReadyAt());
        assertEquals(t0.plus(Duration.ofMinutes(80)), engine.get(patch).getLatestReadyAt());

        clock.setNow(t0.plus(Duration.ofMinutes(81)));
        engine.tick();
