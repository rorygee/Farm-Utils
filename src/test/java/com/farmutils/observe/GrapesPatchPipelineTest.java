...
public class GrapesPatchPipelineTest
{
    @Test
    public void grapesDecodeAndFeedInference()
    {
        assertTrue(Varbit4953GrapesVarbitObserver.isVineryRegionId(7223));
        assertFalse(Varbit4953GrapesVarbitObserver.isVineryRegionId(7222));

        // Grapes stage 1 (Time Tracking value 2)
        DecodedPatchState stage1 = StandardGrapesSlotDecoder.decode(2);
        ...
        assertEquals(Integer.valueOf(ItemID.GRAPES), stage1.getCropItemIdOrNull());
        ...
        PatchId patch = PatchId.SPECIAL_GRAPES_PLOT_1;
        engine.onObservation(Observation.planted(patch, t0, ObservationSource.VARBIT));
        engine.onObservation(Observation.growthStageObserved(patch, stage1.getStage(), stage1.getMaxGrowthStageOrZero(), Duration.ofMinutes(5), t0, ObservationSource.VARBIT));
        ...
        clock.setNow(t0.plus(Duration.ofMinutes(35)));
        engine.tick();
        assertEquals(InferredStage.GROWING, engine.get(patch).getStage());
        assertEquals(t0.plus(Duration.ofMinutes(35)), engine.get(patch).getEarliestReadyAt());
        assertEquals(t0.plus(Duration.ofMinutes(40)), engine.get(patch).getLatestReadyAt());

        clock.setNow(t0.plus(Duration.ofMinutes(41)));
        engine.tick();
